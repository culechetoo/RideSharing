...
int getLocalPos(int x, int y) {
    return (nV-(1+x)/2.0)*x+(y-x)-1;
}

double localDist(int x, int y){
    if (x==y) {
        return 0;
    }
    else if (x>y) {
        return adjmat[getLocalPos(y, x)];
    }
    else {
        return adjmat[getLocalPos(x, y)];
    }
}
...
void readFromFiles(string locFileName, string edgeFileName, string hstFileName, string inputFileName) {
	ifstream locFile(locFileName.c_str(), ios::in);
	ifstream inputFile(inputFileName.c_str(), ios::in);
	ifstream edgeFile(edgeFileName.c_str(), ios::in);
...
void dumpResult(string& fileName, const char *algo) {
	...
	for(int j=0; j<nW; j++){
		...
		sumTime += st;
		maxCap = max(maxCap, workers[j].picked);
		minCap = min(minCap, workers[j].picked);
	}
	...
	if (!fileName.empty()) {
		...
		fprintf(fout, "%s %.6lf %.6lf %.6lf %.6lf %d %d %.6lf %.6lf\n",
					algo, sumflow, maxflow, sumTime, maxTime, maxCap, minCap, usedTime, usedMemory/1024.0);
		
		fclose(fout);
	} else {
		printf("%s %.6lf %.6lf %.6lf %.6lf %d %d %.6lf %.6lf\n",
					algo, sumflow, maxflow, sumTime, maxTime, maxCap, minCap, usedTime, usedMemory/1024.0);
